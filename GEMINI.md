...
- **Two contracts.** This repo's `AGENTS.md` governs work *on EAAO*; a generated project is
  governed by [`templates/AGENTS.md.tmpl`](templates/AGENTS.md.tmpl) rendered into it.
- **The five-step loop:** Interview → Resolve profile → Write manifest (confirm) → Render →
  Verify & draft PR. See `AGENTS.md` §5 and [`orchestrator/generate.md`](orchestrator/generate.md).
