...
（不含 CI、changelog 或 git 历史）。在**你项目仓库的根目录**解压，其内容 —— `.eaao-core/` 以及
...
tar xzf /tmp/pgs-eaao-bundle.tar.gz   # 将 .eaao-core/、AGENTS.md 等解压到当前目录
```

现在你应当得到 `my-project/.eaao-core/`（与 `AGENTS.md` 并列）。想要 ZIP 或用浏览器？从
[最新发布](https://github.com/danielPoloWork/pgs-eaao/releases/latest)下载任一资产，并在**仓库根目录**
解压（内容位于顶层 —— 无外层文件夹）。随后按 [Quickstart](#quickstart) 确认并生成：

```bash
ls .eaao-core                           # orchestrator/ templates/ tools/ …
python .eaao-core/tools/eaao_lint.py    # 可选：确认工厂内部一致
```
