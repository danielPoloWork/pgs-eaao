...
This is the routine that keeps EAAO modern. It is run by the
...
3. Prove it: `python tools/eaao_lint.py` (profile completeness) **and** a render of a manifest
...
Schedule a recurring cloud agent with the `/schedule` skill so EAAO checks itself on a cadence
without you remembering to. A monthly routine, for example, whose prompt is:

> *"Adopt the profile-author role (`.eaao-core/agent/profile-author.md`). For each
> `.eaao-core/orchestrator/profiles/<lang>.yaml`, check whether its tool versions, CI runner
> images, action pins, and default standard are still current; if not, draft a one-profile-per-PR
> update with an ADR where a default tool changes. Run `.eaao-core/tools/eaao_lint.py` and a
