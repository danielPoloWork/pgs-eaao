...
EAAO's agents do not retrain a model; they improve through **structured, versioned memory**.
...
1. **Read first.** At the start of a generation run (or a task on EAAO), the agent reads
...
Each entry (validated by `tools/eaao_lint.py`'s `lessons` check):
